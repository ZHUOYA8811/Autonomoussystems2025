 ...
 */
   ...
   current_sample_time_(0.0)
 {
   ...
   double health_period = this->declare_parameter<double>("health_report_period_sec", 1.0);
 
   // Publisher
   command_pub_ = this->create_publisher<trajectory_msgs::msg::MultiDOFJointTrajectory>(
     mav_msgs::default_topics::COMMAND_TRAJECTORY, 1);
   pub_node_health_ = this->create_publisher<state_machine::msg::Answer>("statemachine/node_health", 10);
   ...
   timer_health_report_ = this->create_wall_timer(
    std::chrono::duration<double>(health_period),
    [this]() {
      state_machine::msg::Answer msg;
      msg.node_name = "sampler"; 
      msg.state = 1; // RUNNING
      this->pub_node_health_->publish(msg);
    });
