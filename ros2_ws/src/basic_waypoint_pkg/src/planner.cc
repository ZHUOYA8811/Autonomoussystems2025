  ...
  max_ang_a_(0.0)
{
  ...
  // Waypoint inputs
  const std::vector<double> default_waypoint_positions;
  node_->declare_parameter<std::vector<double>>(
    "waypoints.positions", default_waypoint_positions);
...
bool BasicPlanner::hasOdometry() const
{
  return has_odom_;
}
  ...
  mav_trajectory_generation::Trajectory* trajectory)
{
  ...
  // - If external goal_pos is provided: fly DIRECTLY to goal (no intermediate waypoints).
  //   Inserting all yaml waypoints as intermediates would force the drone to visit every
  //   waypoint before reaching the commanded target, which is wrong for TAKEOFF/LAND etc.
  // - If external goal_pos is NOT provided (fallback yaml route): use all waypoints except
  //   the last as intermediate points, and last waypoint becomes the end vertex.
  if (has_waypoints) {
    const size_t middle_count = has_goal_pos ? 0
